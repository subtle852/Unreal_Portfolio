...
void UAN_StartLying::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, const FAnimNotifyEventReference& EventReference)
{
	...
	if (IsValid(MeshComp) == true)
	{
		...
		{
			AttackingCharacter->OnStartLying();
		}
		
		AGMonster* Monster = Cast<AGMonster>(MeshComp->GetOwner());
		if (IsValid(Monster) == true)
		{
			Monster->OnStartLying();
		}
	}
}
